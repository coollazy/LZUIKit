...
extension UICollectionView {
    private static var swizzleImplementation: Void = {
        let originalMethod = class_getInstanceMethod(UICollectionView.self, #selector(layoutSubviews))
        let swizzledMethod = class_getInstanceMethod(UICollectionView.self, #selector(collection_view_empty_content_view_layoutSubviews))
        if let originalMethod = originalMethod, let swizzledMethod = swizzledMethod {
            method_exchangeImplementations(originalMethod, swizzledMethod)
        }
        else {
            print("UICollectionView enableEmptyContentView => Failed")
        }
    }()

    @objc func collection_view_empty_content_view_layoutSubviews() {
        collection_view_empty_content_view_layoutSubviews()

        // 預設會自動將 emptyContentView 的大小設定跟 UICollectionView 一樣
        ...
        if isItemsCountEqualToZero() {
            ...
            if emptyContentView?.isHidden == false {
...
extension UICollectionView {
    ...
    public var emptyContentView: UIView? {
        get {
            let key: StaticString = "UICollectionView+EmptyContentView_emptyContentView"
